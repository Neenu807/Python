...
def get_slot_machine_spins(rows,cols,symbols):
    ...
    for col in range(cols):
        column=[]
        current_symbols=all_symbols[:]   # :used to copy values in a list
...
def print_slot_machine(columns):
    for row in range(len(columns[0])):      # taking transpose
        for i,column in enumerate(columns):
            if i != len(columns)-1 :        #checking i not equals max index
...
def spin(balance):
    ...
    while True:
        ...
        if total_bet>balance:
            print(f"your bet amount is greater than balance.balance :${balance}") 
        else:
            break
    print(f"you are betting ${bet} on lines {lines}.thus total amount equals:${total_bet}")
    print(balance,lines)
